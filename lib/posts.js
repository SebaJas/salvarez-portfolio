...
// `posts` is capped for the index; `total` is the real count, so the caller knows
// whether a "See all" link is needed.
export function getSections(options) {
  const posts = getAllPosts(options);

  return SECTIONS.map((section) => {
    const all = posts.filter((post) => post.section === section.slug);
    return { ...section, posts: all.slice(0, POSTS_PER_SECTION), total: all.length };
  }).filter((section) => section.total > 0);
}

// Every post in one section, uncapped — for /blog/section/<slug>.
export function getSectionPosts(slug, options) {
  return getAllPosts(options).filter((post) => post.section === slug);
}
